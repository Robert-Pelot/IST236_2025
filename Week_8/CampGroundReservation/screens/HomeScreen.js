...
function HomeScreen() {
  ...
  const [numCampSites, setNumCampSites] = useState(1);
  ...
  const onChangeNumCampSites = (itemValue) => {
    setNumCampSites(itemValue);
  ...
  function onReserveHandler() {
    ...
    res = res + `Camp Sites:\t\t${campSitesCounts[numCampSites - 1]}\n`;
    setResults(res);
  }
  ...
  const showNumCampSitesPicker = () => setShowNumCampSites(true);

  return (
    <ImageBackground
      source={require("../assets/images/mcintosh-lake-campground.jpg")}
      resizeMode="cover"
      ...
      >
        ...
        <ScrollView style={styles.scrollContainer}>
          <View style={styles.titleContainer}>
            <Title>Fuzzy Navel Campground</Title>
          </View>
          ...
          <View style={styles.rowContainer}>
            ...
            <Pressable onPress={showNumCampSitesPicker}>
              <Text style={[styles.dateText, dateTextFlex]}>
                {campSitesCounts[numCampSites - 1]} Sites
              </Text>
            </Pressable>
          </View>

          <Modal animationType="slide" transparent={true} visible={showNumCampSites}>
            <View style={styles.centeredModalView}>
              <View style={styles.modalView}>
                <Text>Enter Number of Camp Sites:</Text>
                <Picker
                  selectedValue={campSitesCounts[numCampSites - 1]}
                  onValueChange={onChangeNumCampSites}
                  style={{ height: 200, width: 200 }}
                >
                  {campSitesCounts.map((count, index) => (
                    <Picker.Item
                      label={count.toString()}
                      value={count}
                      key={index}
